...
<title>마이스터 판매글</title>
  ...
  <link href="/resources/bh/sell-css/showSell.css?after" rel="stylesheet">
...
<div class="row" style="width:1110px; padding-top:30px;">
  ...
  <div class="col-lg-8">
    ...
    <p class="no-font">${sell.sellvo.sellCategory1 } > ${sell.sellvo.sellCategory2 }</p>
    <!-- Preview Image -->
    <img class="img-fluid rounded" src="/resources/upload/sellImg/${sell.sellvo.sellImg }">
    <!-- Post Content -->
    <hr>
    <div id="navbar" style="width:710px;">
  ...
  <!-- Sidebar Widgets Column -->
   ...
   <%--  <c:if test="${sessionScope.member.memberNo != sell.pickvo.memberNo}">
    ...
    <div class="card my-4 card-shadow">
     ...
     <h5 class="fo-si-13">마이스터을 통해 결제하면 거래 완료시까지 결제 대금을 안전하게 보호 받을 수 있습니다.</h5>
     </div>
    </div>
    <!-- Side Widget -->
    <div class="card my-4 card-shadow">
    	<div class="card-body" style="text-align:center;">
    		...
    		<div class="card-time">
    			...
    			<c:if test="${not empty sessionScope.member.memberNo }">
    				<h5 class="fo-si-13">마이스터 전화 : ${sell.membervo.memberPhone }</h5>
    				<h5 class="fo-si-13">연락 가능한 시간 : ${sell.membervo.memberTime }</h5>
    			</c:if> 
    			<hr>
    		</div>
    		<div class="card-Info">
    		<h5 style="text-align:left;" class="fo-si-15 fo-we-bo">마이스터 소개</h5>
    		<p class="fo-si-14" style="text-align:left;">${sell.membervo.memberIntro }</p>
    		</div>
    	</div>
