...
<link rel="shortcut icon" type="image⁄x-icon"
	href="/resources/yr/imgs/logo.png">
							...
							<div class="row no-gutters">
								<div class="col-lg-3 col-xl-2 border-right"
									...
									<div class="card-body border-bottom">
										<form>
											<input id="searchForm" class="form-control" type="text"
												...
												<div class="message-center">
													...
													<c:forEach items="${list }" var="c">
														<a href="javascript:void(0)"
															...
															<div class="user-img">
																<c:if test="${empty c.senderImg}">
																<img src="/resources/upload/common/none_user.png"
																	alt="user" class="img-fluid rounded-circle"
																	style="width:40px;height:40px;">
																</c:if>	
																<c:if test="${not empty c.senderImg}">
																<img src="/resources/upload/memberImg/${c.senderImg }"
																	alt="user" class="img-fluid rounded-circle"
																	style="width:40px;height:40px;">
																</c:if>	
										...
										<div class="row">
											...
											<div class="col-3">
												<label id="sendLabel"
													...
													aria-hidden="true"></i>파일업로드</label> <a
													style="background: #ffbc42; color: white;"
	...
	<script>
											...
											success : function(data) {
												...
												for (var i = 0; i < data.length; i++) {
													if ("${sessionScope.member.memberNickname}" == data[i].chatSender) {
														...
														html += `<div style="font-size: 17px;border-radius:10px;padding:0.5rem 1.2rem !important;" class="box msg p-2 d-inline-block mb-1 box">`
																+ data[i].chatContent
																+ `</div>`
														html += `<br></div>`;
														html += `<div style="font-size:12px;" class="chat-time text-right d-block font-10 mt-1 mr-0 mb-3">`;
														var arr = data[i].chatTime
																.split("/");
														html += `<div class="chat-gap" style="display:none;">`
																+ arr[0]
																+ "/"
																+ arr[1]
																+ "/"
														...
														html += `<h6 class="font-weight-medium">`
																+ data[i].chatSender
																+ `</h6>`;
														html += `<div style="font-size: 17px;border-radius:10px;padding:0.5rem 1.2rem !important;" class="msg p-2 d-inline-block mb-1">`
																+ data[i].chatContent
																+ `</div>`;
														html += `</div>`;
		...
		function connect() {
			...
			ws.onmessage = function(e) {
				...
				html += `<h6 class="font-weight-medium">` + data.sender
						+ `</h6>`;
				html += `<div style="font-size: 17px;border-radius:10px; padding:0.5rem 1.2rem !important;" class="msg p-2 d-inline-block mb-1">` + data.msg
						+ `</div>`;
				html += `</div>`;
				html += `<div style="font-size:12px;" class="chat-time d-block font-10 mt-1 mr-0 mb-3">`
		...
		function transChat(param) {
			...
			if (param == undefined) {
				...
				var arr = param.split(":");
				var extensions = [ ".jpg", ".gif", ".png", ".JPG", ".PNG",
						".GIF" ];
				msg = `<a href="/meister/member/chatFileDownload.do?filename=`
						+ arr[0] + `&filepath=` + arr[1]
						+ `"style="color:#ffbc42;font-weight:bold;text-decoration:underline;">`;
				msg += arr[0];
			...
			if (msg !== "" || param != undefined) {
				...
				let minutes = today.getMinutes() < 10 ? "0"
						+ today.getMinutes() : today.getMinutes(); // 분
				...
				var target = $(".message-center>a").eq(clickIndex).find("h6")
						.html();
				var sendMsg = {
		...
		$(function(){
			$("#searchForm").keyup(function(){
				 ...
				 if ($(this).val() != "") {
			            ...
			            $(".message-item").css("display", "block")
			        }
			})
		})
	</script>
